...
def send_file(file_path, root_path, s):
    ...
    size = int(file_size)
    num_buffer = int(size / BUFFER)
    reminder = int(size % BUFFER)
...
def create_file(combined_path, file_size, s):
    ...
    try:
        ...
        with open(combined_path, "wb") as f:
            # receive data in chunks of BUFFER and write it to the file.
            for i in range(num_buffer):
                received_content = s.recv(BUFFER)
                f.write(received_content)

            # receive and write the reminder.
            received_content = s.recv(reminder)
            f.write(received_content)
            s.send("ack".encode())
        # close the file.
        f.close()
